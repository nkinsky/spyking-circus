...
SHARED_MEMORY = hasattr(MPI.Win, "Allocate_shared") and callable(
    getattr(MPI.Win, "Allocate_shared")
)
...
def gather_array(data, mpi_comm, root=0, shape=0, dtype="float32", compress=False):
    ...
    if not compress:
        ...
        mpi_comm.Gatherv(
            [data.flatten(), size, mpi_type],
            [gdata, (sizes, displacements), mpi_type],
            root=root,
        )
    else:
        ...
        if comm.rank == root:
            for blosc_data in data:
                gdata.append(
                    numpy.frombuffer(blosc.decompress(blosc_data), dtype=np_type)
                )
...
def all_gather_array(data, mpi_comm, shape=0, dtype="float32", compress=False):
    ...
    if not compress:
        gdata = numpy.empty(numpy.int64(sum(sizes)), dtype=np_type)
        mpi_comm.Allgatherv(
            [data.flatten(), size, mpi_type], [gdata, (sizes, displacements), mpi_type]
        )
    else:
        data = blosc.compress(data, typesize=mpi_type.size, cname="blosclz")
