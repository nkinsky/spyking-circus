...
def check_if_done(params, flag, logger):
    ...
    if value == "false":
        ...
        particular_sentence = (
            "And set the flag %s in the [noedits] section to False" % flag
        )
        if comm.rank == 0:
            if flag == "filter_done":
                msg = [
                    "Code was interrupted while filtering",
                    common_sentence,
                    particular_sentence,
                ]
            elif flag == "artefacts_done":
                msg = [
                    "Code was interrupted while removing artefacts",
                    common_sentence,
                    particular_sentence,
                ]
            elif flag == "median_done":
                msg = [
                    "Code was interrupted while removing median",
                    common_sentence,
                    particular_sentence,
                ]
            elif flag == "ground_done":
                msg = [
                    "Code was interrupted while removing ground",
                    common_sentence,
                    particular_sentence,
                ]
            else:
...
def main(
    ...
    def filter_file(
        ...
        if nb_shanks > 1:
            ...
            for i in list(params.probe["channel_groups"].keys()):
                shank_channels[i] = numpy.array(
                    params.probe["channel_groups"][i]["channels"], dtype=numpy.int32
                )
        else:
        ...
        if comm.rank == 0:
            ...
            if do_remove_ground:
                to_write += [
                    "Channels %s are used as reference channels in respective shanks"
                    % common_ground
                ]
        ...
        if flag_saturation:
            ...
            saturation_channels = open(
                file_out_suff + ".channels-%d.data" % comm.rank, "wb"
            )
            saturation_values = open(
                file_out_suff + ".values-%d.data" % comm.rank, "wb"
            )

            if data_file_in.data_dtype in [
                "float32",
                numpy.float32,
                "float64",
                numpy.float64,
                ...
                max_value = (
                    numpy.iinfo(data_file_in.data_dtype).max - data_file_in.dtype_offset
                )
        ...
        for count, gidx in enumerate(to_explore):
            ...
            if flag_saturation:
                ...
                if sat_threshold == "negative":
                    ...
                    indices = numpy.where(
                        numpy.abs(local_chunk) >= saturation * data_file_in.gain
                    )
                ...
                to_keep = (times >= padding[0]) & (
                    times < (len_chunk - numpy.abs(padding[1]))
                )
                sub_times = times[to_keep]
                sub_channels = channels[to_keep]

                saturation_times.write(
                    (sub_times + t_offset).astype(numpy.uint32).tostring()
                )
            ...
            if do_filtering:
                if not process_all_channels:
                    local_chunk[:, nodes] = signal.filtfilt(
                        b, a, local_chunk[:, nodes], axis=0
                    )
            ...
            if do_remove_median:
                if nb_shanks == 1:
                    ...
                    for i in list(params.probe["channel_groups"].keys()):
                        global_median = numpy.median(
                            numpy.take(local_chunk, shank_channels[i], axis=1), 1
                        )
                        local_chunk[:, shank_channels[i]] -= global_median[
                            :, numpy.newaxis
                        ]
    ...
    def compute_artefacts(data_file):
        ...
        artefacts = numpy.loadtxt(
            params.get("triggers", "trig_file"), comments=["#", "//"]
        )
        windows = numpy.loadtxt(
            params.get("triggers", "trig_windows"), comments=["#", "//"]
        )
        ...
        mytest = numpy.all(
            numpy.in1d(numpy.unique(artefacts[:, 0]), numpy.unique(windows[:, 0]))
        )

        if not mytest:
            if comm.rank == 0:
                print_and_log(
                    ["Error in the trigger file: not all artefacts are defined"],
                    "error",
                    logger,
                )
            sys.exit(0)
        ...
        mask = (all_times >= 0) & (
            all_times + numpy.max(windows[:, 1]) < data_file.t_stop
        )
        ...
        for count, artefact in enumerate(local_labels):
            ...
            if len(numpy.where(numpy.diff(times) < tau)[0]) > 0:
                if comm.rank == 0:
                    print_and_log(
                        ["Stimulation times for artefact %d are too close!" % artefact],
                        "error",
                        logger,
                    )
                sys.exit(0)
    ...
    def remove_artefacts(data_file, art_dict):
        ...
        if not mytest:
            if comm.rank == 0:
                print_and_log(
                    ["Error in the trigger files: not all artefacts are defined"],
                    "error",
                    logger,
                )
            sys.exit(0)
    ...
    if params.getboolean("data", "overwrite"):
        ...
        comm.Barrier()
        data_file_out = params.get_data_file(
            is_empty=not has_been_created, params=description
        )
    ...
    if clean_artefact:
        if not (
            os.path.exists(params.get("triggers", "trig_file"))
            and os.path.exists(params.get("triggers", "trig_windows"))
        ):
            if comm.rank == 0:
                print_and_log(
                    ["trig_file or trig_windows file can not be found"], "error", logger
                )
            sys.exit(0)
